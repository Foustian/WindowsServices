                ...
                foreach (XElement xElem in xDoc.Descendants("SubMedia"))
                {
                    ...
                    if (xElem.Parent.Name == "NM" && xElem.Descendants("Content").FirstOrDefault().Value == "1")
                    {
                        ...
                        else
                        {
                            ...
                            UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, xElem.Parent.Name.ToString(), newsContent);
                        }
                    }
                    else if (xElem.Parent.Name == "SM" && xElem.Descendants("Content").FirstOrDefault().Value == "1")
                    {
                        ...
                        else
                        {
                            ...
                            UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, xElem.Parent.Name.ToString(), smContent);
                        }
                    }
                    else if (xElem.Parent.Name == "TV")
                    {
                        if (xElem.Descendants("CC").FirstOrDefault().Value == "1")
                        {
                            ...
                            else
                            {
                                ...
                                UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, xElem.Parent.Name.ToString(), tvContent);
                            }
                        }
                    ...
                    else if (xElem.Parent.Name == "PQ" && xElem.Descendants("Content").FirstOrDefault().Value == "1")
                    {
                        ...
                        else
                        {
                            ...
                            UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, xElem.Parent.Name.ToString(), pqContent);
                        }
                    }
                    else if (xElem.Parent.Name == "IQR")
                    {
                        if (xElem.Descendants("CC").FirstOrDefault().Value == "1")
                        {
                            string radioContent = feedReportLogic.DoHttpGetRequest(ConfigurationManager.AppSettings["TVContentURL"] + xElem.Descendants("ArticleID").FirstOrDefault().Value);
                            if (string.IsNullOrWhiteSpace(radioContent))
                            {
                                radioContent = string.Empty;
                                xElem.Descendants("CC").FirstOrDefault().Value = "1";
                                LogMessage("INFO", "content is empty for Radio media _ArchiveMediaID:" + xElem.Descendants("ID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                            }
                            else
                            {
                                xElem.Descendants("CC").FirstOrDefault().Value = "0";
                                radioContent = new System.Data.SqlTypes.SqlXml(System.Xml.XmlReader.Create(new System.IO.StringReader(radioContent))).Value;
                                LogMessage("INFO", "Radio closed caption :" + (radioContent.Length > 300 ? radioContent.Substring(0, 300) + "...." : radioContent), p_ArchiveMetaDataUpdateTask.ID);
                                LogMessage("INFO", "update archive metadata content for Radio _ArchiveMediaID: " + xElem.Descendants("ID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                                UpdateArchiveMediaRecord(xElem.Descendants("ID").FirstOrDefault().Value, xElem.Parent.Name.ToString(), radioContent);
                            }
                        }

                        if (xElem.Descendants("Export").FirstOrDefault().Value == "1")
                        {
                            LogMessage("INFO", "call export service for Radio media guid : " + xElem.Descendants("ArticleID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                            string exportResponse = feedReportLogic.DoHttpGetRequest(ConfigurationManager.AppSettings["TVExportURL"] + xElem.Descendants("ArticleID").FirstOrDefault().Value);
                            string statusCode = String.Empty;
                            string responseMessage = "No response";
                        ...
                        if (xElem.Descendants("IOSExport").FirstOrDefault().Value == "1")
                        {
                            LogMessage("INFO", "call IOSExport service for Radio media guid : " + xElem.Descendants("ArticleID").FirstOrDefault().Value, p_ArchiveMetaDataUpdateTask.ID);
                            string iosExportResponse = feedReportLogic.DoHttpGetRequest(ConfigurationManager.AppSettings["TVIOSExportURL"] + xElem.Descendants("ArticleID").FirstOrDefault().Value);
                            if (iosExportResponse.Contains(ConfigurationManager.AppSettings["TVIOSExportSuccessMessage"]) || iosExportResponse.Contains(ConfigurationManager.AppSettings["TVIOSExportAlreadyQueuedMessage"]))
                            {
                                xElem.Descendants("IOSExport").FirstOrDefault().Value = "0";
